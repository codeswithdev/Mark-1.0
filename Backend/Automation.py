...
classes = ["zGubwf", "hgK8Ic", "LTKOO sY7ryc", "Z0LcW", "gsrt vk_bk FzvW5b YwMwnf", "pclize", "tw-data-text tw-text-small tw-ta",
           "IZ6rdc", "O5uR6d LTKOO", "vl_rd", "webanswers-webanswers_table__webanswers-table", "dDoNo ikb4bb gsrt", "sXla0e",
           "lwHnbc", "VQF4g", "qv3No", "kno-rdesc", "SPZ7db"]
...
def OpenApp(app, sess=requests.session()):
    # 1. Common Websites ka Shortcut (Best way)
    links_dict = {
    ...
    # Agar website list mein hai toh direct open karo
    if app_lower in links_dict:
        webopen(links_dict[app_lower])
        return True

    # 2. Agar website nahi hai, toh System App check karo
    try:
        appopen(app, match_closest=True, output=True, throw_error=True)
        return True
    except:
        # 3. Agar App bhi nahi hai, toh Google se dhoondo
        ...
        if html:
            ...
            # Google ke naye structure ke hisaab se links dhoondna
            links = [a['href'] for a in soup.find_all('a', href=True) if "url?q=" in a['href']]
            
            if links:
                # Pehla valid link nikalna
                full_link = links[0].split("url?q=")[1].split("&")[0]
                webopen(full_link)
            else:
